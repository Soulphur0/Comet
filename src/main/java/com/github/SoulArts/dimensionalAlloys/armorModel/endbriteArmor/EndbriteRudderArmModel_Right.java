...
public class EndbriteRudderArmModel_Right<T extends LivingEntity> extends AnimalModel<T> {
    private final ModelPart bb_main;

    public EndbriteRudderArmModel_Right() {
        textureWidth = 16;
        textureHeight = 16;
        bb_main = new ModelPart(this);
        bb_main.setPivot(-11.0F, 10.0F, -2.0F);
        bb_main.setTextureOffset(0, 0).addCuboid(-2.0F, -11.0F, 0.0F, 4.0F, 7.0F, 1.0F, 0.0F, false);
    }
    ...
    public void setAngles(T livingEntity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch){
        ...
        this.bb_main.pitch = MathHelper.cos(limbSwing * 0.6662F + 3.1415927F) * 1.0F * limbSwingAmount * 0.25F / k;
